...
##### Q: How can you determine if an object is part of the prototype chain of another object?

```js
if (obj1.isPrototypeOf(obj2)) {
  // obj1 is in the prototype chain of obj2
}
```
...
1. What will the following code output? Try to determine the results without running the code.
   ...
   `undefined` because in strict mode the implicit context for function calls is `undefined` instead of the global object.
...
2. What will the following code output? Explain the difference, if any, between this output and that of problem 1.
   ...
   this outputs `obj` because `func()` is invoked as a method by object `obj`. The implicit execution context for method invocation is the calling object. 
...
4. Now we can build a factory function to create payments. The function can take an object argument in one of 3 forms:
   ...
   Hint: default values should be 0.
...
2. What happens if you run the following code? Why?
   ...
   This solution throws an error because `scamper` is an undefined property on `lizzy`. Since `Lizard` was invoked without the  `new` operator, it is invoked as a regular function call. `this` on line 2 thus refers to `undefined` instead of `lizzy`. There is no explicit return value, so the return value is `undefined`, and that is the value of `lizzy`. We cannot call methods on `undefined`, so it throws an error.
...
1. What does the following code log to the console? Try to answer without running the code. Can you explain why the code produces the output it does?
   ...
   We get `NaN` because we are performing operations on `undefined` which results in `NaN`.
...
4. What will the following code log to the console and why?
   ...
   Objects hold reference to their prototype objects. If the object's prototype changes in some way, the changes are reflected in the inheriting object. Even tho we defined swingSword on ninja's prototype object after we created ninja, it becomes available to the `ninja` object. 
